...
export class NotesController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: "Create a new secure note with the userID" })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: "Get all notes belonging to the current user" })
  findAll(@User() user: UserPrisma) {
    return this.notesService.findAll(user);
  }

  @Get(':id')
  @ApiParam({ name: "id", description: "ID to get a specific note", example: 8 })
  @ApiOperation({ summary: "Get a specific note by userID" })
  @ApiResponse({ status: HttpStatus.OK, description: "Get a object with the specific note that belongs to the current user" })
  findOne(@Param('id', ParseIntPipe) id: number, @User() user: UserPrisma) {
    return this.notesService.findOne(user, id);
  }

  @Delete(':id')
  @ApiParam({ name: "id", description: "ID to delete a specific note", example: 8 })
  @ApiOperation({ summary: "Delete a specific note by userID" })
  @ApiResponse({ status: HttpStatus.NO_CONTENT, description: "Get the specific note that belongs to the current user" })
